...
import id.co.bni.ets.lib.base.controller.table.AbstractCrudTableController;
...
/**
 * Facade class to map admin crud with table operation and join {@code LogApi} with {@code Mapping}.
 * <p>
 ...
 * @since 1.0.0.RELEASE
 */
public abstract class AbstractAdminController<T extends AbstractTrackedEntity<ID>, ID extends Serializable, R>
        extends AbstractCrudTableController<T, ID, R>
        implements AdminCrudAuthorize<T, ID, R>, AdminTableAuthorize<T> {

    protected <S extends CrudServiceOperation<T, ID> & TableServiceOperation<T>> AbstractAdminController(S service) {
        super(service);
    }
